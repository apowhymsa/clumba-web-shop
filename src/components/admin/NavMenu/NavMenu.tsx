...
import {
    MdKeyboardArrowDown, MdKeyboardArrowUp, MdOutlineDeliveryDining, MdOutlineExitToApp, MdOutlineMailOutline
} from 'react-icons/md';
...
const NavMenu = () => {
    ...
    return <>
        <ul className="flex flex-wrap gap-y-2 p-6 bg-[#383f51] gap-x-4 w-full text-[14px]">
            <li ref={ref} className="flex flex-col rounded flex-1 gap-x-4 bg-white transition-colors cursor-pointer">
                ...
                <ul className="submenu flex-col flex px-4">
                    <li className="flex items-center rounded gap-x-4 bg-white w-full transition-colors cursor-pointer hover:bg-rose-200">
                        <Link href="/admin/ingCategories" className="px-4 flex-1 h-10 grid items-center">Категорії
                            інгредієнтів</Link>
                    </li>
                    <li className="flex items-center rounded gap-x-4 bg-white w-full transition-colors cursor-pointer hover:bg-rose-200">
                        <Link href="/admin/ingredients"
                              className="px-4 flex-1 h-10 grid items-center">Інгредієнти</Link>
                    </li>
                    <li className="flex items-center rounded gap-x-4 bg-white w-full transition-colors cursor-pointer hover:bg-rose-200">
                        <Link href="/admin/productCategories" className="px-4 flex-1 h-10 grid items-center">Категорії
                            товарів</Link>
                    </li>
                    <li className="flex items-center rounded gap-x-4 bg-white w-full transition-colors cursor-pointer hover:bg-rose-200">
            ...
            <li onClick={() => setOpenDeliveryModal(true)} className="flex items-center flex-1 justify-between rounded gap-x-4 bg-white transition-colors cursor-pointer h-10 hover:bg-rose-200">
                <div className="flex gap-x-4 px-4 flex-1 h-10 items-center">
                    <span><MdOutlineDeliveryDining/></span>
