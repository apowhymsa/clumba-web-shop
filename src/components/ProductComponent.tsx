...
import "../app/products/[slug]/Product.scss";
...
const ProductComponent = (props: ProductComponentProps) => {
  const { productObject, slug } = props;
  ...
  const [product, setProduct] = useState<Product | null>(productObject);
  ...
  useEffect(() => {
    ...
    const getComments = async () => {
      ...
      const { data } = await axios.get(
        `${process.env.ADMIN_ENDPOINT_BACKEND}/comments/${slug}`,
        requestConfig
      );
    ...
    getRecomendations(productObject.categoryID._id);

    Promise.all([getComments()]).finally(() => setLoading(false));
  }, []);
