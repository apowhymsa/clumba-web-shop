...
const Page = ({ params }: { params: { slug: string } }) => {
    ...
    const getOrder = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.ADMIN_ENDPOINT_BACKEND}/order/${params.slug}`,
          {
            method: "GET",
            headers: {
        ...
        const ids = data.products.map((product: any, index: number) => {
          return {
            productID: product.product_id._id,
            variantID: product.productVariant.id,
          };
        });
        ...
        const productsByIds = await axios.post(
          `${process.env.ADMIN_ENDPOINT_BACKEND}/product-variant-by-id`,
          requestBody,
          requestConfig
        );
  ...
  async function handleChangeStatus() {
    try {
      ...
      info(
        `Статус замовлення ${response.data.payment.liqpayPaymentID} оновлено на ${requestBody.status}`
      );
    } catch (err: unknown) {
      const errObject = err as any;
      console.log(error);
  ...
  return (
    <div id="liqpay_checkout" className="shadow mx-6 my-6 p-5 bg-white">
      {order ? (
        <>
            ...
            >
              ...
              <div className="flex flex-col gap-y-2">
                ...
                {order.status === "complete" ? (
                  <span>
                    Замовлення має статус: <strong>&#34;Завершено&#34;</strong>
                  </span>
                ) : (
                      ...
                      >
                        <div className="flex gap-x-4">
                          <div className="w-[120px] h-[120px]">
                            ...
                            <img
                              ...
                              src={`${process.env.ADMIN_ENDPOINT_BACKEND}/images/${item.product_id.image}`}
                              alt="Image"
                            />
                          </div>
                        ...
                        <div>
                          ...
                          {item.product_id.variants
                            .find(
                              (variant: any) =>
                                variant._id === item.productVariant.id
                            )
                            ?.ingredients.map((ing: any, index: number) => (
                              <p key={index}>
                                {ing.ingredient.id.title} -{" "}
                                {ing.ingredient.variantID.vType} - {ing.count}
                              </p>
                            ))}
                        </div>
                      </div>
