...
const Page = () => {
    ...
    async function createProduct(data: FormValues, event: BaseSyntheticEvent<object, any, any> | undefined) {
        ...
        const normalizeIngredients = data.variants.map((variant: IVariant) => {
            return {title: variant.title, ingredients: variant.ingredients.map(ingredient => {
                return {
            ...
            <div className="flex flex-col gap-y-2">
                ...
                <div className="flex gap-x-4 items-center">
                    ...
                    <div className="flex flex-2 items-center space-x-3 self-end mb-[10px]">
                        <label htmlFor="price" className="relative inline-flex cursor-pointer items-center">
                            <input {...register('discount.state')} onChange={(e) => {
                                setDiscount({state: e.target.checked, percent: discount.percent});
                            }} type="checkbox" id="price" className="peer sr-only h-8 text-sm"/>
                            <div
                    ...
                    <div className="flex-1">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                className={`block w-full h-8 text-sm rounded-md shadow-sm pl-8 ${errors.discount?.percent ? "border-red-300 focus:border-red-300 focus:ring focus:ring-red-200" : "border-gray-300 focus:border-blue-300 focus:ring focus:ring-blue-200"}  focus:ring-opacity-50 disabled:cursor-not-allowed disabled:bg-gray-50 disabled:text-gray-500`}
                                {...register("discount.percent", {
                                    min: {
                                        value: 0, message: 'Ціна товару повинна бути вища за 0'
                                    }, max: {
                                        value: 100, message: 'Ціна товару повинна бути не вища за 100'
                                    }
                                })}
                            />
                        </div>
                        {errors.discount?.percent ? (
                            <p className="mt-1 text-sm text-red-500">{errors.discount?.percent.message}</p>) : null}
                    </div>
                </div>
                <div>
