...
const Page = () => {
  ...
  const toImage = (base64: string) => {
    ...
    const fileType = (product?.image.name as string).endsWith(".png")
      ? "image/png"
      : "image/jpeg";
  ...
  async function updateProduct(
      ...
      return {
        ...
        ingredients: variant.ingredients.map((ingredient) => {
          return {
            ...
            ingredient: {
              id: (ingredient.ingredient.id as any).hasOwnProperty("_id")
                ? (ingredient.ingredient.id as any)._id
                : (ingredient.ingredient.id as any).value,
              variantID: (
                ingredient.ingredient.variantID as any
              ).hasOwnProperty("_id")
                ? (ingredient.ingredient.variantID as any)._id
                : (ingredient.ingredient.variantID as any).value,
            },
          };
        }),
      };
    ...
    try {
      const response = await axios.put(
        `${process.env.ADMIN_ENDPOINT_BACKEND}/product/${searchParams.get(
          "id"
        )}`,
        requestBody,
        requestConfig
      ...
      switch (errorObject.response?.status) {
        case 403: {
          error(
            "Не отриманно необхідних даних для оновлення запису, оновіть сторінку та повторіть спробу"
          );
          break;
        }
        case 409: {
  ...
  return (
    <div className="p-4 m-4 min-h-screen bg-white">
      ...
      >
        <div className="flex flex-col gap-y-2">
          ...
          <div className="flex flex-2 items-center space-x-3 my-2">
            ...
            >
              <input
                defaultChecked={product.isNotVisible}
            ...
            <div className="flex gap-x-4">
              {prodImage ? (
                <div className="flex h-[200px]">
                  <img
                    src={prodImage.isNew ? prodImage.base64 : prodImage.url}
                    alt="Image"
          ...
          <div>
            ...
            {fields.map((variant, variantIndex) => (
              <div key={variant.id}>
                  ...
                  <div className="flex flex-col flex-1 gap-y-2">
                    ...
                    <div className="flex gap-x-4 items-center">
                      ...
                      <div className="flex flex-2 items-center space-x-3 self-end mb-[10px]">
                        ...
                        >
                          <input
                            defaultChecked={variant.discount.state}
                            {...register(
                ...
                <NestedFieldArrayUpdate
                  defaultIngredients={variant.ingredients}
        ...
        <div className="flex gap-x-4 w-max mt-4">
          <Button
            type="submit"
            variant="primary"
            content={isLoadingUpdate ? <Loader /> : "Оновити запис"}
